...
int main(void)
{
    ...
    for(i=0; i < SIMPLE_TILES_LENGTH; i++)
    {
        CHARBLOCKBASE[0].tileimg[i] = simple_tiles[i];
    }

    for(i=0; i<SIMPLE_PALETTE_LENGTH; i++)
    {
        PALETTE[i] = simple_palette[i];
    }
    ...
    REG_BG0CNT = GAMEMAP_MAP_TYPE | SBB(28) | CBB(0) | SIMPLE_PALETTE_TYPE;
    REG_BG0HOFS = 0;
    REG_BG0VOFS = 0;
